...
interface Drawer{
   ...
   * 打开一个抽屉
   * @param options
   * @deprecated legacy 设置为 false 时返回 index, 默认返回当前抽屉 DOM 对象
   */
  open(options?: Options): Index | this
  /**
   * 关闭指定的抽屉
   * @param index
   * @deprecated legacy 设置为 false 时使用 `drawer.close(index)`, 默认用 drawer.open 返回的抽屉对象调用 close 方法关闭
   */
