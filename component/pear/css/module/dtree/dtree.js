...
layui.define(["jquery", "layer", "form"], function (exports) {
  ...
  DTree.prototype.setting = function (options) {
    ...
    this.success =
      this.options.success || OPTIONS.success || function (data, obj) {}; //树加载完毕后执行解析树之前的回调
          ...
          if (code == _this.response.statusCode) {
            ...
            if (_this.dataStyle == "layuiStyle") {
              ...
              _this.obj.html(
                _this
                  .getNoneDom()
                  .errText(result.status[_this.response.message])
              );
      ...
      AjaxHelper.request({
        ...
        success: function (result) {
          ...
          if (code == _this.response.statusCode) {
            ...
            var level = parseInt(_this.node.level) + 1;
            if (_this.dataFormat == "list") {
              var pListData = _this.queryListTreeByPid(
                pid,
                result[_this.response.rootName]
              );
              _this.loadListTree(
                pListData,
                result[_this.response.rootName],
  ...
  DTree.prototype.chooseDataInit = function (chooseIds) {
    ...
    for (var i = 0; i < chooseId.length; i++) {
      _this.obj
        .find("i[dtree-click='" + eventName.checkNodeClick + "']")
        .each(function () {
      ...
      $(
        ...
        $(
          "div[dtree-id='" +
            ...
            _this.selectCardDiv +
            "']"
        ).toggleClass("dtree-select-show layui-anim layui-anim-upbit");
