layui.define(["jquery", "element"], function (exports) {
  ...
  pearTab.prototype.render = function (opt) {
    ...
    if (option.session) {
      if (sessionStorage.getItem(option.elem + "-pear-tab-data") != null) {
        tabData = JSON.parse(
          sessionStorage.getItem(option.elem + "-pear-tab-data")
        );
        option.data = JSON.parse(
          sessionStorage.getItem(option.elem + "-pear-tab-data")
        );
        tabDataCurrent = sessionStorage.getItem(
          option.elem + "-pear-tab-data-current"
        );
    ...
    option.success(
      sessionStorage.getItem(option.elem + "-pear-tab-data-current")
    );
  ...
  pearTab.prototype.click = function (callback) {
    ...
    element.on("tab(" + this.option.elem + ")", function (data) {
      ...
      if (!option.preload) {
        var $iframe = $(
          ".layui-tab[lay-filter='" + elem + "'] .layui-tab-content"
        ).find("iframe[id='" + id + "']");
  ...
  pearTab.prototype.addTab = function (opt) {
    ...
    sessionStorage.setItem(
      this.option.elem + "-pear-tab-data",
  ...
  pearTab.prototype.addTabOnlyByElem = function (elem, opt, time) {
    ...
    if (
      $(".layui-tab[lay-filter='" + elem + "'] .layui-tab-title li[lay-id]")
        .length <= 0
    ) {
      element.tabAdd(elem, {
        title: title,
  ...
  pearTab.prototype.addTabOnly = function (opt, time) {
    ...
    if (
      ...
      sessionStorage.setItem(
        this.option.elem + "-pear-tab-data-current",
        opt.id
      );
    } else {
      var isData = false;
  ...
  function tabIframeLoading(elem, id) {
    var load =
      ...
      "</div>";
    var $iframe = $(
      ".layui-tab[lay-filter='" + elem + "'] .layui-tab-content .layui-show"
    ).find("iframe");
    if (id) {
      $iframe = $(
        ".layui-tab[lay-filter='" + elem + "'] .layui-tab-content"
  ...
  function createTab(option) {
    ...
    $.each(option.data, function (i, item) {
      ...
      if (option.index == index) {
        ...
        TitleItem +=
          '<li lay-id="' + item.id + '" ><span class="pear-tab-active"></span>';
      }
      ...
      if (item.close) {
        ...
        // 当 前 选 项 卡 不 允 许 关 闭
        TitleItem +=
          '<span class="disable-close title">' + item.title + "</span>";
      }
      TitleItem +=
  ...
  function toolEvent(option) {
    $("body .layui-tab[lay-filter='" + option.elem + "']").on(
      "click",
      "#closeThis",
      function () {
        var currentTab = $(
