...
(function (layui, window, factory) {
  ...
  Common.prototype.createDD = function (id, item, clz) {
    ...
    let attr =
      pid[0] == -1
        ? ""
        : `tree-id="${pid.join("-")}" tree-folder="${!!item[
            "XM_TREE_FOLDER"
          ]}"`;
    return `<dd lay-value="${item.value}" class="${
      item.disabled ? DISABLED : ""
            ...
            item.disabled ? DISABLED : ""
          }"  style="margin-left: ${(pid.length - 1) * 30}px">
    ...
    $(target ? target : document)
      ...
      .on("click", `dl.${DL}`, (e) => {
        ...
        if (dd.hasClass(FORM_SELECT_TIPS)) {
          ...
          let obj = data[id].config.btns.filter(
            (bean) => bean.name == method
          )[0];
          obj &&
            obj.click &&
  ...
  Common.prototype.linkageAdd = function (id, val) {
    ...
    do {
      pid = vs[index];
      li = dl.find(
        `.xm-select-linkage-group${index + 1} li[xm-value="${pid}"]`
      );
      li[0] && lis.push(li);
      index++;
  ...
  Common.prototype.selectAll = function (id, isOn, skipDis) {
    ...
    dl.find(
      `dd[lay-value]:not(.${FORM_SELECT_TIPS}):not(.${THIS})${
    ...
    $.each(target, (key, val) => {
      ...
      if (btns.length) {
        ...
        let html = common.renderBtns(
          key,
          show,
          config && config.space ? config.space : "30px"
        );
        dd.html(html);
      } else {
