...
layui.define(["jquery", "element"], function (exports) {
  ...
  var _createContainer = function (watermark) {
    ...
    watermark._container.style.cssText =
      "display: block; pointer-events: none;";
    watermark._container.setAttribute("aria-hidden", true);
    _parentEle =
      document.querySelector(watermark.options.appendTo) || document.body;
    //获取页面最大宽度
  ...
  var _createWatermark = function (watermark, options) {
    ...
    let offsetLeft =
      _left +
      (_windowsWidth - options.width * cols - options.colSpacing * (cols - 1)) /
        2;
    let offsetTop =
      _top +
      (_windowsHeight -
        options.height * rows -
        options.rowSpacing * (rows - 1)) /
        2;
  ...
  var _addResizeListener = function (watermark) {
    _resizeHandler = function () {
      //获取页面最大宽度
      var _windowsWidth_n = Math.max(
        _parentEle.scrollWidth,
        _parentEle.clientWidth
      );
      //获取页面最大高度
      var _windowsHeight_n = Math.max(
        _parentEle.scrollHeight,
        _parentEle.clientHeight
      );
      ...
      if (
        _windowsHeight_n !== _windowsHeight ||
        _windowsWidth_n !== _windowsWidth
      ) {
  ...
  function Watermark(options = {}) {
    ...
    _proto.upload = function (content) {
      ...
      for (const watermark of this._container.querySelectorAll(
        ".cell-watermark"
      )) {
