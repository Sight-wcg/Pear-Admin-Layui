layui.define(["jquery", "layer", "form"], function (exports) {
    ...
    cloneObj: function (obj, filter) {
      ...
      for (var key in obj) {
        if (str.indexOf(key) == -1) {
          ...
          newObj[key] =
            typeof val === "object"
              ? event.cloneObj(val, typeof filter !== undefined ? filter : [])
              : val;
        }
      }
      return newObj;
  ...
  DTree.prototype.setting = function (options) {
    ...
    this.iconfontStyle =
      this.options.iconfontStyle || OPTIONS.iconfontStyle || {}; //用于自定义树的每个关键部位使用的图标
    this.nodeIconArray =
      $.extend(
        nodeIconArray,
        this.options.nodeIconArray || OPTIONS.nodeIconArray
      ) || nodeIconArray; //用户自定义非叶子节点图标集合，node
    this.leafIconArray =
      $.extend(
        leafIconArray,
        this.options.leafIconArray || OPTIONS.leafIconArray
    ...
    this.defaultRequest =
      $.extend(
        this.defaultRequest,
        this.options.defaultRequest || OPTIONS.defaultRequest
      ) || this.defaultRequest; //默认请求参数
    this.filterRequest =
      this.options.filterRequest || OPTIONS.filterRequest || []; //过滤请求参数
    this.request = this.options.request || OPTIONS.request || {}; //用户自定义请求参数
    this.response =
      $.extend(this.response, this.options.response || OPTIONS.response) ||
      this.response; //返回json格式
    this.data = this.options.data || OPTIONS.data || null; //初始化指定该参数，则不会访问异步接口
    this.dataFormat =
      this.options.dataFormat || OPTIONS.dataFormat || "levelRelationship"; //用于用户配置的data数据格式，list：列表，  levelRelationship：层级关系，默认
    this.dataStyle =
      this.options.dataStyle || OPTIONS.dataStyle || "defaultStyle"; //用于用户配置layui通用的json数据风格,layuiStyle:layui风格，defaultStyle：默认风格
    this.errDataShow = this.options.errDataShow || OPTIONS.errDataShow || false; //是否在递归数据出现错误后，显示错误信息，默认false
    this.withCredentials =
      this.options.withCredentials || OPTIONS.withCredentials || false; //是否允许跨域请求，默认false
    this.beforeSend =
    ...
    this.success =
      this.options.success || OPTIONS.success || function (data, obj, first) {}; //树加载完毕后执行解析树之前的回调
    this.done =
      this.options.done || OPTIONS.done || function (data, obj, first) {}; //树加载完毕后的回调
    this.formatter =
      $.extend(this.formatter, this.options.formatter || OPTIONS.formatter) ||
      this.formatter; //数据过滤
    this.error =
      this.options.error ||
      OPTIONS.error ||
      function (XMLHttpRequest, textStatus, errorThrown) {}; // 异步加载异常回调
    this.complete =
      this.options.complete ||
      OPTIONS.complete ||
      function (XMLHttpRequest, textStatus) {}; // 异步加载完成回调

    /** 复选框参数**/
    this.checkbar = this.options.checkbar || OPTIONS.checkbar || false; //是否开启复选框模式
    this.checkbarLoad =
      this.options.checkbarLoad || OPTIONS.checkbarLoad || "node"; //复选框作用范围，node：所有节点， leaf：最后一级；默认所有节点
    this.checkbarType =
      this.options.checkbarType || OPTIONS.checkbarType || "all"; //复选框选中形式	all：子集选中父级也选中，  no-all：子集选中父级半选中，子集全选父级选中，p-casc：父级选中子集全选，子集无法改变父级选中状态， self：没有任何级联关系，only：只能选中一个复选框。   默认all
    this.checkbarData =
      this.options.checkbarData || OPTIONS.checkbarData || "choose"; //复选框记录数据类型形式，  change表示记录变更数据，choose表示记录选中数据，all记录全部数据，halfChoose记录选中和半选中的数据，默认choose
    this.checkbarFun =
      $.extend(
        this.checkbarFun,
        this.options.checkbarFun || OPTIONS.checkbarFun
    ...
    this.menubarTips =
      $.extend(
        this.menubarTips,
        this.options.menubarTips || OPTIONS.menubarTips
      ) || this.menubarTips; //菜单栏吸附， toolbar：依附在工具栏，group：依附在按钮组，freedom，自由
    this.menubarFun =
      $.extend(
        this.menubarFun,
        this.options.menubarFun || OPTIONS.menubarFun
      ) || this.menubarFun; //menubar事件加载

    /** 工具栏参数**/
    this.toolbar = this.options.toolbar || OPTIONS.toolbar || false; //是否开启可编辑模式
    this.toolbarWay =
      this.options.toolbarWay || OPTIONS.toolbarWay || "contextmenu"; //工具栏显示方式，contextmenu:右键，follow:跟随节点，fixed:固定在节点右侧
    this.toolbarStyle =
      $.extend(
        this.toolbarStyle,
        this.options.toolbarStyle || OPTIONS.toolbarStyle
      ) || this.toolbarStyle; //toolbar的自定义风格，标题，弹框大小
    this.toolbarLoad =
      this.options.toolbarLoad || OPTIONS.toolbarLoad || "node"; //toolbar作用范围：node:所有节点，noleaf:非最后一级节点，leaf:最后一级
    this.toolbarShow = this.options.toolbarShow ||
      OPTIONS.toolbarShow || ["add", "edit", "delete"]; //toolbar三个按钮自定义加载
    ...
    this.toolbarFun =
      $.extend(
        this.toolbarFun,
        this.options.toolbarFun || OPTIONS.toolbarFun
      ) || this.toolbarFun; //toolbar事件加载
    ...
    this.iframeDefaultRequest =
      $.extend(
        this.iframeDefaultRequest,
        this.options.iframeDefaultRequest || OPTIONS.iframeDefaultRequest
  ...
  DTree.prototype.reloadSetting = function (options) {
    ...
    this.cache =
      typeof this.options.cache === "boolean" ? this.options.cache : this.cache; //开启数据缓存
    this.record =
      typeof this.options.record === "boolean"
        ? this.options.record
        : this.record; //开启数据记录模式
    this.load =
      typeof this.options.load === "boolean" ? this.options.load : this.load; //开启加载动画
    ...
    /** 样式相关参数**/
    this.line =
      typeof this.options.line === "boolean" ? this.options.line : this.line; //开启树线，默认不开启
    ...
    this.nodeIconArray =
      $.extend(nodeIconArray, this.options.nodeIconArray) || this.nodeIconArray; //用户自定义非叶子节点图标集合，node
    this.leafIconArray =
      $.extend(leafIconArray, this.options.leafIconArray) || this.leafIconArray; //用户自定义叶子节点图标集合，leaf
    this.skin = this.options.skin || this.skin; //自定义样式
    if (this.skin == "layui") {
      //layui主题
      this.line =
        typeof this.options.line === "boolean" ? this.options.line : true; //开启树线，默认开启
      ...
      //laySimple主题
      this.line =
        typeof this.options.line === "boolean" ? this.options.line : false; //开启树线，默认不开启
    ...
    this.async =
      typeof this.options.async === "boolean" ? this.options.async : this.async; //异步同步加载,默认异步加载
    ...
    this.defaultRequest =
      $.extend(this.defaultRequest, this.options.defaultRequest) ||
      this.defaultRequest; //默认请求参数
    ...
    this.response =
      $.extend(this.response, this.options.response) || this.response; //返回json格式
    ...
    this.errDataShow =
      typeof this.options.errDataShow === "boolean"
        ? this.options.errDataShow
        : this.errDataShow; //是否在使用list模式递归数据出现错误时，显示错误信息
    this.withCredentials =
      typeof this.options.withCredentials === "boolean"
    ...
    this.formatter =
      $.extend(this.formatter, this.options.formatter) || this.formatter; //数据过滤
    ...
    this.checkbarFun =
      $.extend(this.checkbarFun, this.options.checkbarFun) || this.checkbarFun; //checkbar事件加载

    /** 菜单栏参数**/
    this.menubar = this.options.menubar || this.menubar; //是否打开菜单栏
    this.menubarTips =
      $.extend(this.menubarTips, this.options.menubarTips) || this.menubarTips; //菜单栏吸附， toolbar：依附在工具栏，group：依附在按钮组，freedom，自由
    this.menubarFun =
      $.extend(this.menubarFun, this.options.menubarFun) || this.menubarFun; //menubar事件加载
    ...
    this.toolbarStyle =
      $.extend(this.toolbarStyle, this.options.toolbarStyle) ||
      this.toolbarStyle; //toolbar的自定义风格，标题，弹框大小
    ...
    this.toolbarFun =
      $.extend(this.toolbarFun, this.options.toolbarFun) || this.toolbarFun; //toolbar事件加载
    ...
    this.iframeDefaultRequest =
      $.extend(this.iframeDefaultRequest, this.options.iframeDefaultRequest) ||
      this.iframeDefaultRequest; //iframe的默认传递参数
    this.iframeRequest =
      $.extend(this.iframeRequest, this.options.iframeRequest) ||
      this.iframeRequest; //iframe的自定义参数
    this.iframeFun =
      $.extend(this.iframeFun, this.options.iframeFun) || this.iframeFun; //iframe事件加载
  ...
  DTree.prototype.selectSetting = function () {
    ...
    this.selectInitVal =
      this.obj.attr("data-value") || this.options.selectInitVal || ""; //输入框的值
  ...
  DTree.prototype.reloadSelectSetting = function () {
    this.selectInitVal =
      this.obj.attr("data-value") ||
      this.options.selectInitVal ||
      this.selectInitVal; //输入框的值
    this.selectTips = this.options.selectTips || this.selectTips; // 输入框的提示语
    this.selectCardHeight =
      this.options.selectCardHeight || this.selectCardHeight; // 下拉面板的高度
    this.selectInputName =
      $.extend(this.selectInputName, this.options.selectInputName) ||
      this.selectInputName; // select表单中的元素
  ...
  DTree.prototype.reloadSelectDom = function () {
    ...
    $(
      "div[dtree-id='" + rootId + "'][dtree-card='" + _this.selectCardDiv + "']"
  ...
  DTree.prototype.selectVal = function (param) {
    ...
    $("div[dtree-select='" + _this.selectDiv + "']")
      .find(
        "input[dtree-id='" + rootId + "'][name='" + _this.selectTipsName + "']"
      )
  ...
  DTree.prototype.ensureTheme = function () {
    ...
    for (var i = 0; i < iconfonts.length; i++) {
      ...
      if (typeof ifontStyle !== "undefined") {
        ...
        this.useDefaultOrUserDefineMenubarExtStyle(
          ifont,
          ifontStyle.menubarExt
        );
        this.useDefaultOrUserDefineToolbarStyle(ifont, ifontStyle.toolbar);
        this.useDefaultOrUserDefineToolbarExtStyle(
          ifont,
          ifontStyle.toolbarExt
        );
      }
    }
  };
  ...
  DTree.prototype.useDefaultOrUserDefineFnodeStyle = function (ifont, fnode) {
    ...
    if (typeof fnode === "undefined") {
      this.usefontStyle.fnode.node.open =
        tempOpen == ""
          ? ifont + " " + this.nodeIconArray[this.fnodeIcon]["open"]
          : tempOpen; // 一级图标中的node节点open图标
      this.usefontStyle.fnode.node.close =
        tempClose == ""
          ? ifont + " " + this.nodeIconArray[this.fnodeIcon]["close"]
          : tempClose; // 一级图标中的node节点close图标
      this.usefontStyle.fnode.leaf =
        tempLeaf == ""
          ? ifont + " " + this.leafIconArray[this.fleafIcon]
  ...
  DTree.prototype.useDefaultOrUserDefineSnodeStyle = function (ifont, snode) {
    ...
    if (typeof snode === "undefined") {
      this.usefontStyle.snode.node.open =
        tempOpen == ""
          ? ifont + " " + this.nodeIconArray[this.nodeIcon]["open"]
          : tempOpen; // 二级图标中的node节点open图标
      this.usefontStyle.snode.node.close =
        tempClose == ""
          ? ifont + " " + this.nodeIconArray[this.nodeIcon]["close"]
          : tempClose; // 二级图标中的node节点close图标
      this.usefontStyle.snode.leaf =
        tempLeaf == ""
          ? ifont + " " + this.leafIconArray[this.leafIcon]
   ...
   */
  DTree.prototype.useDefaultOrUserDefineCheckboxStyle = function (
    ifont,
    checkbox
  ) {
    var _this = this;
    ...
    if (typeof checkbox === "undefined") {
      this.usefontStyle.checkbox.on =
        tempOn == "" ? ifont + " " + LI_DIV_CHECKBAR_ON : tempOn;
      this.usefontStyle.checkbox.out =
        tempOut == "" ? ifont + " " + LI_DIV_CHECKBAR_OUT : tempOut;
      this.usefontStyle.checkbox.noall =
        tempNoall == "" ? ifont + " " + LI_DIV_CHECKBAR_NOALL : tempNoall;
    } else {
      var on = checkbox.on;
   ...
   */
  DTree.prototype.useDefaultOrUserDefineMenubarStyle = function (
    ifont,
    menubar
  ) {
    var _this = this;
    ...
    if (typeof menubar === "undefined") {
      ...
      this.usefontStyle.menubar.moveup =
        tempMoveup == "" ? ifont + " " + LI_DIV_MENUBAR_UP : tempMoveup;
      this.usefontStyle.menubar.refresh =
        tempRefresh == "" ? ifont + " " + LI_DIV_MENUBAR_REFRESH : tempRefresh;
      this.usefontStyle.menubar.checkAll =
        tempCheckAll == ""
          ? ifont + " " + LI_DIV_MENUBAR_CHECKALL
          : tempCheckAll;
      this.usefontStyle.menubar.unCheckAll =
        tempUncheckAll == ""
          ? ifont + " " + LI_DIV_MENUBAR_UNCHECKALL
          : tempUncheckAll;
      this.usefontStyle.menubar.invertAll =
        tempInvertAll == ""
          ? ifont + " " + LI_DIV_MENUBAR_INVERTALL
          : tempInvertAll;
      this.usefontStyle.menubar.remove =
        tempRemove == "" ? ifont + " " + LI_DIV_MENUBAR_DELETE : tempRemove;
      this.usefontStyle.menubar.search =
        tempSearch == "" ? ifont + " " + LI_DIV_MENUBAR_SEARCH : tempSearch;
    } else {
   ...
   */
  DTree.prototype.useDefaultOrUserDefineMenubarExtStyle = function (
    ifont,
    menubarExt
  ) {
    var _this = this;
   ...
   */
  DTree.prototype.useDefaultOrUserDefineToolbarStyle = function (
    ifont,
    toolbar
  ) {
    var _this = this;
    ...
    if (typeof toolbar === "undefined") {
      this.usefontStyle.toolbar.menubar.movedown =
        tempMovedown == ""
          ? ifont + " " + this.usefontStyle.menubar.movedown
          : tempMovedown;
      this.usefontStyle.toolbar.menubar.moveup =
        tempMoveup == ""
          ? ifont + " " + this.usefontStyle.menubar.moveup
          : tempMoveup;
      this.usefontStyle.toolbar.menubar.refresh =
        tempRefresh == ""
          ? ifont + " " + this.usefontStyle.menubar.refresh
          : tempRefresh;
      this.usefontStyle.toolbar.menubar.checkAll =
        tempCheckAll == ""
          ? ifont + " " + this.usefontStyle.menubar.checkAll
          : tempCheckAll;
      this.usefontStyle.toolbar.menubar.unCheckAll =
        tempUnCheckAll == ""
          ? ifont + " " + this.usefontStyle.menubar.unCheckAll
          : tempUnCheckAll;
      this.usefontStyle.toolbar.menubar.invertAll =
        tempInvertAll == ""
          ? ifont + " " + this.usefontStyle.menubar.invertAll
          : tempInvertAll;
      this.usefontStyle.toolbar.menubar.remove =
        tempRemove == ""
          ? ifont + " " + this.usefontStyle.menubar.remove
          : tempRemove;
      this.usefontStyle.toolbar.menubar.search =
        tempSearch == ""
          ? ifont + " " + this.usefontStyle.menubar.search
          : tempSearch;
      this.usefontStyle.toolbar.menubarExt =
        tempExt == "" ? this.usefontStyle.menubarExt : tempExt;
      this.usefontStyle.toolbar.pulldown =
        tempPulldown == ""
          ? ifont + " " + LI_DIV_TOOLBAR_PULLDOWN
          : tempPulldown;
      this.usefontStyle.toolbar.pullup =
        tempPullup == "" ? ifont + " " + LI_DIV_TOOLBAR_PULLUP : tempPullup;
      this.usefontStyle.toolbar.add =
        tempAdd == "" ? ifont + " " + LI_DIV_TOOLBAR_ADD : tempAdd;
      this.usefontStyle.toolbar.edit =
        tempEdit == "" ? ifont + " " + LI_DIV_TOOLBAR_EDIT : tempEdit;
      this.usefontStyle.toolbar.del =
        tempDel == "" ? ifont + " " + LI_DIV_TOOLBAR_DEL : tempDel;
    } else {
   ...
   */
  DTree.prototype.useDefaultOrUserDefineToolbarExtStyle = function (
    ifont,
    toolbarExt
  ) {
    var _this = this;
    ...
    AjaxHelper.request({
      ...
      success: function (result) {
        ...
        if (code == _this.response.statusCode) {
          ...
          if (_this.dataStyle == "layuiStyle") {
            _this.obj.html(
              _this.getNoneDom().errText(result[_this.response.message])
            );
            _this.error(null, code, result[_this.response.message]);
          } else {
            _this.obj.html(
              _this.getNoneDom().errText(result.status[_this.response.message])
            );
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        // 异步加载异常回调
        _this.obj.html(
          _this.getNoneDom().errText(textStatus + ": " + errorThrown)
        );
  ...
  DTree.prototype.loadListTree = function (nowListData, listData, level, $ul) {
    ...
    if (nowListData.length > 0) {
      for (var i = 0; i < nowListData.length; i++) {
        ...
        var childListData = _this.queryListTreeByPid(
          parseData.treeId(),
          listData
        ); // 根据已知数据的id判断该条数据是否还有子数据
    ...
    return {
      ...
      fmtTitle: function () {
        if (typeof _this.formatter.title === "function") {
          ...
          tt =
            ftitle == "" || ftitle == undefined || ftitle == null ? tt : ftitle;
          return tt || "";
        }
      ...
      disabled: function () {
        return typeof data[_this.response.disabled] === "boolean"
          ? data[_this.response.disabled]
          : false;
      },
      hide: function () {
        return typeof data[_this.response.hide] === "boolean"
          ? data[_this.response.hide]
          : false;
      },
      checkArr: function () {
        var checkArr = [];
      ...
      basicData: function () {
        return (
          event.escape(JSON.stringify(data[_this.response.basicData])) ||
          JSON.stringify({})
        );
      },
      recordData: function () {
          ...
          if (checkArr && checkArr.length > 0) {
            for (var i = 0; i < checkArr.length; i++) {
              ...
              if (checked == "2") {
                //半选择
                CHOOSE_CLASS =
                  _this.usefontStyle.checkbox.noall + " " + _this.style.chs;
              } else if (checked == "1") {
                //选择
                CHOOSE_CLASS =
                  _this.usefontStyle.checkbox.on + " " + _this.style.chs;
              } else {
                //未选择或者无值
   ...
   * @Param: hide： 是否隐藏
   */
  DTree.prototype.replaceDom = function (
    $div,
    treeId,
    last,
    spread,
  ...
  DTree.prototype.dataInit = function (chooseId) {
    ...
    if ($div && $div.length > 0) {
      ...
      $li_parents
        ...
        .children(
          "i[data-spread]." +
            event.trimToDot(_this.usefontStyle.fnode.node.close)
        )
      ...
      $li_parents
        ...
        .children(
          "i[data-spread]." +
            event.trimToDot(_this.usefontStyle.snode.node.close)
        )
  ...
  DTree.prototype.setDisabledAllNodes = function () {
    var _this = this;
    _this.obj
      .find("div[dtree-click='" + eventName.itemNodeClick + "']")
      .each(function () {
        var $div = $(this);
  ...
  DTree.prototype.getDisabledNodesParam = function (disabledIds) {
    ...
    if (disabledIds.length) {
      for (var i = 0; i < disabledId.length; i++) {
        ...
        if ($div != null && $div.attr("dtree-disabled") == "true") {
          disabledNodes.push(
            _this.getRequestParam(_this.getTempNodeParam($div))
          );
        }
      }
    } else {
  ...
  DTree.prototype.getAllHideNodesParam = function () {
    ...
    _this.obj
      .find(
        "div[dtree-click='" + eventName.itemNodeClick + "'][dtree-hide='true']"
      )
      .each(function () {
        var $div = $(this);
        hideNodes.push(_this.getRequestParam(_this.getTempNodeParam($div)));
  ...
  DTree.prototype.chooseDataInit = function (chooseIds) {
    ...
    var $li_parents = _this.obj
      .find(
        "i[dtree-click='" + eventName.checkNodeClick + "'][data-checked='1']"
      )
      .parents("." + LI_NAV_ITEM);
    $li_parents.children("ul").addClass(NAV_SHOW);
    $li_parents
      .children("." + LI_DIV_ITEM)
      .children(
        "i[data-spread]." + event.trimToDot(_this.usefontStyle.fnode.node.close)
      )
      .addClass(_this.usefontStyle.fnode.node.open);
    $li_parents
      .children("." + LI_DIV_ITEM)
    ...
    $li_parents
      .children("." + LI_DIV_ITEM)
      .children(
        "i[data-spread]." + event.trimToDot(_this.usefontStyle.snode.node.close)
      )
      .addClass(_this.usefontStyle.snode.node.open);
    $li_parents
      .children("." + LI_DIV_ITEM)
    ...
    if (_this.checkbarData == "change") {
      ...
      _this.obj.find("i[data-par][dtree-disabled='false']").each(function () {
        ...
        if ($i.attr("data-checked") != $i.attr("data-initchecked")) {
          if (requestParamFlag) {
            _this.checkbarNode.push(
              _this.getRequestParam(_this.getCheckbarNodeParam($div, $i))
            );
          } else {
      ...
      //记录全部数据
      _this.obj
        .find("i[data-par][data-checked][dtree-disabled='false']")
        .each(function () {
          var $i = $(this),
            $div = $i.closest("." + LI_DIV_ITEM);
          if (requestParamFlag) {
      ...
      //记录选中数据
      _this.obj
        .find("i[data-par][data-checked='1'][dtree-disabled='false']")
        .each(function () {
          var $i = $(this),
            $div = $i.closest("." + LI_DIV_ITEM);
          if (requestParamFlag) {
      ...
      _this.obj
        .find("i[data-par][data-checked='2'][dtree-disabled='false']")
        .each(function () {
          var $i = $(this),
            $div = $i.closest("." + LI_DIV_ITEM);
          if (requestParamFlag) {
  ...
  DTree.prototype.getCheckbarNodeParam = function ($div, $i) {
    ...
    temp_node.leaf =
      _this.getNodeDom($div).cite().attr("data-leaf") == "leaf" ? true : false;
    temp_node.level = _this.getNodeDom($div).parentLi().attr("data-index");
    temp_node.spread =
      _this.getNodeDom($div).fnode().attr("data-spread") == "open"
        ? true
        : false;
  ...
  DTree.prototype.initTreePlus = function () {
    ...
    if (
      _this.menubar &&
      _this.menubarTips.group &&
      _this.menubarTips.group.length > 0
    )
      _this.obj.before(
        "<div class='" +
          LI_DIV_MENUBAR +
        ...
        if (
          ...
          for (var i = 0; i < _this.menubarTips.group.length; i++) {
            ...
            if (menuId == ext.menubarId) {
              ext.handler(
                _this.getRequestParam(_this.getNodeParam($div), $div)
              );
              break;
            }
          }
      ...
      for (var i = 0; i < toolbarExt.length; i++) {
        ...
        _this.toolbarMenu[ext.toolbarId] = _this
          ...
          .setToolbarOption(
            ext.toolbarId,
            ext.title,
            _this.usefontStyle.toolbarExt + " " + ext.icon,
            ""
          );
      }
    }
    ...
    return {
      ...
      setToolbarPlace: function (toolbarMenu) {
        if (toolbarWay == "contextmenu") {
          ...
          _this.obj
            .find("cite[data-leaf][dtree-disabled='false']")
            .each(function () {
              var $cite = $(this);
  ...
  DTree.prototype.dynamicToolbarDom = function ($cite) {
    ...
    if ($cite.next("em." + TOOLBAR_TOOL_EM).length == 0) {
      ...
      var toolbarMenus = _this.toolbarFun.loadToolbarBefore(
  ...
  DTree.prototype.toolbarHide = function () {
    var _this = this;
    if (_this.toolbar && _this.toolbarWay == "contextmenu") {
      var $toolBarDiv = _this.obj.prevAll(
    ...
    switch (tool) {
      ...
      case defaultTool.addTool:
        ...
        layer.open({
          ...
          success: function (layero, index) {
            form.render();
            form.on(
              "submit(dtree_addNode_" + _this.obj[0].id + "_form)",
              function (data) {
                var data = data.field;
                var parentId = $div.attr("data-id"),
                  id =
                    $div.attr("data-id") + "_node_" + $ul[0].childNodes.length,
                  leaf = true,
                  checked = "0",
      ...
      case defaultTool.editTool:
        ...
        layer.open({
          ...
          content: content,
          success: function (layero, index) {
            _this.toolbarFun.editTreeLoad(
              _this.getRequestParam(_this.getNodeParam($div))
            );
            form.render();
            form.on(
              "submit(dtree_editNode_" + _this.obj[0].id + "_form)",
              function (data) {
                var data = data.field;
      ...
      case defaultTool.addTool:
        ...
        for (var j = 0; j < addNodeBarDef.length; j++) {
          ...
          switch (type) {
            ...
            case "textarea":
              addNodeBar.push(
                _this.loadToolBarDetail(addNodeBarDef[j]).textarea()
              );
              break;
            case "select":
              addNodeBar.push(
                _this.loadToolBarDetail(addNodeBarDef[j]).select()
              );
              break;
            case "hidden":
              addNodeBar.push(
                _this.loadToolBarDetail(addNodeBarDef[j]).hidden()
              );
              break;
          }
        }
      ...
      case defaultTool.editTool:
        ...
        for (var j = 0; j < editNodeBarDef.length; j++) {
          ...
          switch (type) {
            case "text":
              editNodeBar.push(
                _this.loadToolBarDetail(editNodeBarDef[j]).text()
              );
              break;
            case "textarea":
              editNodeBar.push(
                _this.loadToolBarDetail(editNodeBarDef[j]).textarea()
              );
              break;
            case "select":
              editNodeBar.push(
                _this.loadToolBarDetail(editNodeBarDef[j]).select()
              );
              break;
            case "hidden":
              editNodeBar.push(
                _this.loadToolBarDetail(editNodeBarDef[j]).hidden()
              );
              break;
          }
        }
  ...
  DTree.prototype.loadToolBarDetail = function (nodeBarContents) {
    var _this = this;
    var readonly =
      typeof nodeBarContents.readonly === "boolean"
        ? nodeBarContents.readonly
        : false;
    var disabled =
      typeof nodeBarContents.disabled === "boolean"
        ? nodeBarContents.disabled
        : false;
    ...
    var placeholder = nodeBarContents.placeholder
      ? nodeBarContents.placeholder
      : val;
    return {
      text: function () {
        return [
      ...
      hidden: function () {
        return [
          '<input type="hidden" class="layui-input f-input" value="' +
            val +
            '" lay-verify="' +
            verify +
            '" ',
    ...
    return {
      ...
      nowDiv: function () {
        ...
        return _this.obj
          ...
          : _this.obj
              .find(
                "div[dtree-click='" + eventName.itemNodeClick + "'][data-id]"
              )
              .parent()
              .find("." + NAV_THIS);
      },
  ...
  DTree.prototype.callbackData = function () {
    ...
    return {
      ...
      childNode: function ($div) {
        ...
        var $childDivs = $div
          .next("ul")
          .find("li." + LI_NAV_ITEM + " div." + LI_DIV_ITEM);
        var childNode = [];
        if ($childDivs && $childDivs.length > 0) {
          $childDivs.each(function () {
            var $cDiv = $(this);
            childNode.push(
              _this.getRequestParam(_this.getTempNodeParam($cDiv))
            );
          });
        }
        return childNode;
  ...
  DTree.prototype.bindBrowserEvent = function () {
    ...
    _this.obj.on("click", "i[data-spread]", function (event) {
      ...
      layui.event.call(
        this,
        MOD_NAME,
        "changeTree(" + $(_this.obj)[0].id + ")",
        {
          dom: _this.callbackData().dom($i),
          param: _this.callbackData().node(node),
          show:
            _this.callbackData().dom($i).attr("data-spread") == "open"
              ? true
              : false,
        }
      );
    ...
    _this.obj.on(
      ...
      function (event) {
        ...
        if (_this.useIframe) {
          var iframeParam = _this.getFilterRequestParam(
            _this.getIframeRequestParam(node)
          );
    ...
    if (_this.checkbar) {
      ...
      _this.obj.on(
        ...
        function (event) {
          ...
          var $i = $(this),
            $div = $i.closest(
              "div[dtree-click='" + eventName.itemNodeClick + "']"
            ),
            node = _this.getNodeParam($div);
          // 复选框选中前的回调
          var flag = _this.checkbarFun.chooseBefore(
            $i,
    ...
    if (_this.menubar) {
      ...
      _this.obj
        .prevAll("div#dtree_menubar_" + _this.obj[0].id)
        .on("click", "button[d-menu]", function (event) {
          event.stopPropagation();
          _this.toolbarHide();
      ...
      _this.obj
        .prevAll("div#dtree_toolbar_" + _this.obj[0].id)
        .on("click", "a[d-menu]", function (event) {
          event.stopPropagation();
          _this.toolbarHide();
        ...
        _this.obj.on(
          ...
          function (e) {
            ...
            var e = e || window.event,
              ...
              my =
                $div.offset().top -
                _this.obj.closest(_this.scroll).offset().top +
                15;
            ...
            $toolBarDiv
              .find(".layui-nav-child")
              .addClass("layui-anim-fadein layui-show");
    ...
    if (_this.select) {
      ...
      $(
        ...
        var top =
            $(this).offset().top + $(this).outerHeight() - $WIN.scrollTop() - 5,
  ...
  DTree.prototype.unbindBrowserEvent = function () {
    ...
    if (_this.menubar) {
      ...
      if (
        _this.obj
          .closest("body")
          .find("*[dtree-id='" + _this.obj[0].id + "'][dtree-menu]").length > 0
      ) {
        _this.obj
          .closest("body")
          .find("*[dtree-id='" + _this.obj[0].id + "'][dtree-menu]")
    ...
    if (_this.select) {
      ...
      $(
        "div[dtree-id='" +
          ...
          _this.selectCardDiv +
          "']"
      ).removeClass("dtree-select-show layui-anim layui-anim-upbit");
      $(
        "div[dtree-id='" +
          _this.obj[0].id +
          "'][dtree-select='" +
